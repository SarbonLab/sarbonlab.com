/* Аналитика SarbonLab.
   ...
   Цели:
     ...
     checklist_click — запрос чек-листа в WhatsApp (вместе с whatsapp_click)
*/
(function () {
  'use strict';
  ...
  document.addEventListener('click', function (ev) {
    ...
    if (href.indexOf('wa.me') !== -1) {
      params.place = node.getAttribute('data-place')
                   || (node.classList.contains('fab') ? 'fab'
                   : node.classList.contains('btn-sm') ? 'header'
                   : node.closest('.mbar') ? 'mbar'
                   : 'body');
      goal('whatsapp_click', params);
      if (node.getAttribute('data-goal')) { goal(node.getAttribute('data-goal'), params); }
